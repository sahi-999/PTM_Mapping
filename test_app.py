...
def _auto_color(composition: str, mass: float, classification: str) -> str:
    ...
    if classification == "Artefact": return "#6b7280"
...
for uid in sorted(found_unimod_ids):
    ...
    with cols[0]:
        cfg['selected'] = st.checkbox(f"UniMod:{uid.split(':')[-1]}", value=cfg['selected'], key=f"cb_{uid}")
    with cols[1]:
        cfg['label'] = st.text_input("Label", value=cfg['label'], key=f"lbl_{uid}", label_visibility="collapsed")
    with cols[2]:
        cfg['color'] = st.color_picker("Color", value=cfg['color'], key=f"col_{uid}", label_visibility="collapsed")
    with cols[3]:
            ...
            while clean_mod_str:
                ...
                if match:
                    ...
                    config = st.session_state.ptm_configs.get(uid_lower, {
                        'selected': True, 'label': uid_lower.upper(), 'color': "#436da9",
                    })
...
<style>
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .viewport { width: 100%; height: 480px; background-color: #0b0f19; border: 1px solid #1e293b; border-radius: 12px; box-shadow: 0 4px 6px -1px rgb(0 0 0 / 0.5); position: relative; overflow: hidden; }
  ...
  #hover-card { position: absolute; bottom: 16px; left: 16px; right: 16px; background: rgba(15, 23, 42, 0.95); border: 1px solid #334155; border-radius: 8px; padding: 14px; color: #f8fafc; font-size: 13px; display: none; backdrop-filter: blur(8px); box-shadow: 0 10px 15px -3px rgb(0 0 0 / 0.3); z-index: 1000; }
  ...
  #hover-card .card-ptm-detail { font-size: 11px; color: #94a3b8; margin-top: 4px; }
  #hover-card .card-ptm-detail a { color: #60a5fa; text-decoration: none; }
  #selection-badge { display: none; position: absolute; top: 12px; left: 12px; background: rgba(244,63,94,0.18); border: 1px solid #f43f5e; border-radius: 6px; padding: 6px 10px; color: #fda4af; font-size: 12px; font-weight: 600; z-index: 999; cursor: pointer; user-select: none; }
  #selection-badge:hover { background: rgba(244,63,94,0.32); }
  #seq-row { width: 100%; border: 1px solid #e2e8f0; border-radius: 12px; background: #ffffff; box-shadow: 0 4px 6px -1px rgb(0 0 0 / 0.05); overflow: hidden; }
  ...
  #seq-header h3 { margin: 0; color: #1e293b; font-size: 15px; }
  #seq-filter-note { font-size: 11px; color: #f43f5e; font-weight: 600; display: none; background: rgba(244,63,94,0.08); border: 1px solid #fecdd3; border-radius: 4px; padding: 3px 8px; }
  #sequence-container { height: 180px; overflow-y: auto; padding: 16px 20px; letter-spacing: 8px; line-height: 38px; font-family: 'SFMono-Regular', Consolas, 'Liberation Mono', monospace; font-size: 17px; word-break: break-all; user-select: none; }
</style>
...
stageOverview.loadFile(PDB_URL).then(function(o) {
  ...
  stageOverview.signals.hovered.add(function(proxy) {
    ...
    updateGlobalFocus(atom.resno, false);
  });
...
function _applyOverviewReps(selSeg) {
  ...
  if (selSeg === null) {
    ...
    addBackboneRepr(overviewComp, "polymer", C_GHOST, BACKBONE_STYLE === "cpk" ? 0.06 : 0.12, { cpkScale: 0.14 });
