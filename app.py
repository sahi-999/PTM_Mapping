...
# New: lets the backbone itself be drawn in the same chemically-accurate
# hyperball (stick-and-ball) style used for the PTMs, instead of only
# cartoon/tube. Hyperball is heavier to render across a whole protein, so
# this is a toggle rather than the only option -- and a lower detail level
# is used on the big overview panel vs. the small zoom panel.
...
backbone_style_choice = st.sidebar.radio(
    ...
    ["Cartoon (ribbon — fast)", "Hyperball (atomic stick-and-ball, matches PTMs)"],
    index=0,
    help=(
        "Hyperball renders the real backbone atoms as a stick-and-ball model, "
        "visually matching the PTM representations. It is more expensive to "
        "render than cartoon, especially for large proteins."
    )
)
use_hyperball_backbone = backbone_style_choice.startswith("Hyperball")

# Shared "shrink factor" control: real van der Waals radii are larger than
# typical bond lengths (atoms genuinely overlap in reality), so every
# ball-and-stick viewer (this one included) shrinks sphere size well below
# true VdW scale purely so the bond/stick stays visible. This slider makes
# that convention explicit and adjustable rather than a hidden constant, and
# applies consistently to both the backbone hyperball and the PTM hyperballs
# so the whole structure reads as one consistent chemical model.
atom_scale = st.sidebar.slider(
    "Atom sphere scale (both backbone & PTMs)",
    min_value=0.15, max_value=0.60, value=0.32, step=0.01,
    help=(
        "Fraction of true van der Waals radius used for sphere size. Real VdW "
        "spheres overlap and fuse into a blob at 1.0 scale — this is why every "
        "molecular viewer (PyMOL, Chimera, Jmol, this one) shrinks spheres well "
        "below true scale, purely so the connecting bond stays visible."
    )
)
...
const USE_HB_BACKBONE  = data.useHyperballBackbone;
const ATOM_SCALE       = data.atomScale;
...
//
// NOTE: sphere sizes here (and in the backbone hyperball below) are a
// fraction of true van der Waals radius, controlled by ATOM_SCALE from the
// sidebar. This is the same convention every ball-and-stick viewer uses --
// true VdW spheres physically overlap and would fuse into a blob, hiding
// the bond geometry. Bond lengths/angles themselves are taken directly
// from the real atomic coordinates and are not adjustable/stylized.
function addPtmStructuralReprs(comp, rn, ptm, withLabel = false) {
  ...
  if (compStr.includes("P")) {
    comp.addRepresentation("spacefill", {
      sele: baseSele + " AND .CA", color: color, scale: ATOM_SCALE, opacity: 0.6
    });
  }
  ...
  if (compStr.includes("S") && mass > 30) {
    ["SD", "SG"].forEach(function(atom) {
      comp.addRepresentation("spacefill", {
        ...
        color: "#f0abfc", scale: ATOM_SCALE + 0.08, opacity: 0.8
      });
    });
  }
  ...
  if (compStr.includes("N") && compStr.includes("O") && mass > 30) {
    comp.addRepresentation("spacefill", {
      sele: baseSele + " AND .OH", color: "#fed7aa", scale: ATOM_SCALE + 0.03, opacity: 0.8
    });
  }
...
// ── BACKBONE HYPERBALL RENDERER ─────────────────────────────────────────────
// Renders the real backbone atoms (N, CA, C, O) in the same stick-and-ball
// (hyperball) style as the PTMs, using the same ATOM_SCALE convention so the
// whole structure reads as one consistent chemical model. `radiusType:
// "covalent"` gives physically correct *relative* sizing between elements
// (e.g. sulfur > carbon > nitrogen); absolute scale is still the same
// shrunk-for-legibility convention as everywhere else in this viewer.
// sphereDetail/radialSegments are kept low on the big overview panel (many
// residues) and raised in the zoom panel (few residues) purely for
// rendering performance -- it does not affect geometric accuracy.
...
function _applyOverviewReps(selSeg) {
  ...
  if (selSeg === null) {
    if (USE_HB_BACKBONE) {
      addBackboneHyperball(overviewComp, "polymer", C_BACKBONE, 0.55, true);
    } else {
    ...
    if (allDetSele) {
      if (USE_HB_BACKBONE) {
        addBackboneHyperball(overviewComp, allDetSele, C_PEPTIDE, 0.95, true);
      } else {
    ...
    if (USE_HB_BACKBONE) {
      addBackboneHyperball(overviewComp, "polymer", C_GHOST, 0.1, true);
    } else {
    ...
    if (USE_HB_BACKBONE) {
      addBackboneHyperball(overviewComp, selSele, C_SELECT_HI, 1.0, true);
    } else {
      overviewComp.addRepresentation("cartoon", {
...
function _loadZoomPanel(seg) {
  ...
  stageZoom.loadFile(PDB_URL).then(function(o) {
    ...
    // No "ghost" representation of the rest of the structure here -- the
    // zoom panel only ever adds representations scoped to `selSele`, so
    // nothing outside the chosen residue range is rendered at all.

    // The zoom panel is a small selection, so full hyperball detail here
    // is cheap regardless of the overview's backbone-style toggle -- this
    // is where atomic-level accuracy next to the PTMs matters most.
    addBackboneHyperball(o, selSele, C_SELECT_HI, 1.0, false);
